...
#visualising what training data contains that we are converting a 3d array to 2d array 
%matplotlib inline
...
#visualising the data which is converted to 1d array
import numpy as np
...
input_images = tfc.placeholder(tf.float32, shape=[None, 784])
target_labels = tfc.placeholder(tf.float32, shape=[None, 10])
...
loss_function = tf.reduce_mean(tfc.nn.softmax_cross_entropy_with_logits_v2(logits=digit_weights, labels=target_labels))
